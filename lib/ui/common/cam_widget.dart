...
class CamWidgetState extends State<CamWidget> {
  ...
  asyncInit() async {
    ...
    controller = CameraController(
        cameras[0], ResolutionPreset.high); //Если cameras[1] - будет фронталка
  ...
  @override
  void dispose() {
    // камера слишком много потребляет, поэтому нужно использовать
    controller?.dispose();
    super.dispose();
  }
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        var scale = (min(constraints.maxWidth, constraints.maxHeight) /
            ...
            camera
                .aspectRatio; // Нахождение отношения минимальной к максимальной стороне

        if (scale < 1) scale = 1 / scale;

        return Stack(
          // способ отрисовки виджетов друг на друге
          children: [
            //BottomAppBar(), // Изменить потом вид, как в вк и добавить смену камеры на фронталку
            Transform.scale(
              scale: scale,
              child: Center(
                child: CameraPreview(
